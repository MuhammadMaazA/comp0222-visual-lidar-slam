classdef SystemModel < handle
    ...
    methods(Access = public)
        ...
        function [x, gradFx, gradFv] = predictState(obj, x, u, dT)
            ...
            if (nargout == 3)
                gradFx = eye(l3.trianglebot.SystemModel.NP);
        ...
        function [z, gradHx, gradHm, gradHw, R] = predictSLAMObservation(obj, x, mXY)
            ...
            % ACTIVITY 2: Compute the SLAM observation z = [r; beta]
            
            % Compute bearing (angle to landmark in robot's frame)
            beta = atan2(dXY(2), dXY(1)) - x(3);
            ...
            % ACTIVITY 5: Compute Jacobians for EKF
            if (nargout >= 3)
                ...
                % Jacobian w.r.t. robot state [x, y, θ]
                gradHx = [-dx/r,   -dy/r,    0;      % ∂r/∂[x, y, θ]
                        dy/r2,  -dx/r2,  -1];     % ∂β/∂[x, y, θ]
                
                % Jacobian w.r.t. landmark [m_x, m_y]
                gradHm = [ dx/r,   dy/r;             % ∂r/∂[m_x, m_y]
                        -dy/r2,  dx/r2];           % ∂β/∂[m_x, m_y]
                
                % Jacobian w.r.t. observation noise (identity)
                gradHw = eye(2);
        ...
        function [mXY, gradGx, gradGw, R] = predictLandmarkFromSLAMObservation(obj, x, z)
            ...
            % ACTIVITY 3: Compute landmark position and Jacobians
            
            % Extract measurements
            r = z(1);      % Range
            beta = z(2);   % Bearing
            
            % Compute landmark position (polar to Cartesian + robot position)
            mXY = zeros(2, 1);
            mXY(1) = x(1) + r * cos(phi);  % m_x
            mXY(2) = x(2) + r * sin(phi);  % m_y

            % Set up the optional outputs if requested
            if (nargout >= 3)
                % Jacobian w.r.t. robot state x = [x, y, θ]
                gradGx = [1,  0,  -r * sin(phi);
                        0,  1,   r * cos(phi)];
                
                % Jacobian w.r.t. measurement z = [r, β]
