classdef BaseEdge < g2o.core.HyperGraphElement
    ...
    properties(Access = public, Constant)
        % Special dimension value used to denote the dimension of the
        % measurement of the edge is dynamically set after the object has
        % been constructed.
        DYNAMIC_DIM = -1;
    end
    ...
    properties(Access = protected)
        ...
        % Cached storeage for J'*Omega*J values
        H;

        % Cached storage for J'*Omega*errZ values
        b;
    ...
    methods(Access = protected)
        ...
        function obj = BaseEdge(numVertices, measurementDimension)
            ...
            %       the dimensions of the individual vertices. A value of
            %       -1 means that it will be dynamically set later.
            %
            % Outputs:
            %   obj - (handle)
            ...
            obj.setDimension(measurementDimension);
    ...
    methods(Access = public)
        ...
        function setDimension(obj, measurementDimension)
            % SETDIMENSION Set the measurement dimension of the edge.
            %
            % Syntax:
            %   obj.setDimension(measurementDimension);
            %
            % Description:
            %   Set the measurement dimension of an edge. This is most
            %   commonly used when an edge is created. However, it is a
            %   public method because it is possible to change the
            %   measurement dimension after edge construction. This happens
            %   if the number of observations (e.g., common elements in a
            %   scan) are not known at edge construction time. This method
            %   will throw an exception if (a) the edge has already been
            %   added to a graph and/or (b) the measurementDimension is not
            %   DYNAMIC_DIM.
            %
            % Inputs:
            %   measurementDimension - (int)
            %       The new measurement dimension. If the edge dimension
            %       changes at a later state, use DYNAMIC_DIM.

            % Make sure the edge is not in a graph
            assert(isempty(obj.owningGraph) == true, 'g2o:baseedge:setdimension', ...
                'An attempt was made to change the dimension of an edge which has been registered in a graph');

            % We can set the edge dimension if either it's empty (=just
            % constructed) or has the value DYNAMIC_DIM
            assert (((isempty(obj.dimZ) == true) || (obj.dimZ == g2o.core.BaseEdge.DYNAMIC_DIM)), ...
                'g2o:basedge:setdimension', ...
                'An attempt was made to change the dimension of an edge which has already been set to %d', ...
                obj.dimZ);

            % If the edge is dynamic, return
            if (measurementDimension == g2o.core.BaseEdge.DYNAMIC_DIM)
                obj.dimZ = measurementDimension;
                return
            end
            
            % Check the dimensions are non-negative
            assert(measurementDimension > 0, 'g2o:baseedge:setdimension:invalidmeasurementdimension', ...
                'The measurement dimension must be a non-negative integer; the value is %d', measurementDimension);

            % Store the dimension
            obj.dimZ = measurementDimension;
            
            % We have a good measurement dimension, so we can now allocate
            % the storage
    ...
    methods(Access = public)
        ...
        function setMeasurement(obj, newZ)
            % SETMEASUREMENT Set the measurement for the edge.
            %
            % Syntax:
